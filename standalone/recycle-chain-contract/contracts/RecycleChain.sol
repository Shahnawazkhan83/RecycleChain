...
/// @title RecycleChain - Blockchain-based product lifecycle management
/// @notice Manages product registration, toxic item tracking, and lifecycle updates
/// @dev Uses an owner address and enumerated product states for lifecycle tracking
contract RecycleChain {
    /// @notice Counter for the total number of products created
    uint256 public productCounter;

    /// @notice Contract owner address, set to deployer on deployment
    ...
    /// @notice Enumeration for tracking the status of a product item
    enum ProductStatus {
        MANUFACTURED,
        SOLD,
        RETURNED,
    ...
    /// @notice Defines a product with its metadata and toxic contents
    struct Product {
        uint256 id;
        string name;
        uint256 quantity;
    ...
    /// @notice Structure for toxic items contained within a product
    struct ToxicItem {
        string name;
        uint256 weight;
    }

    /// @notice Structure for each unique product item, with ID and status
    struct ProductItem {
        string id;
        uint256 productId;
    ...
    /// @notice Manufacturer information
    struct Manufacturer {
        string name;
        string location;
        string contact;
    ...
    /// @notice Emitted when a new toxic item is added to a product
    event ToxicItemCreated(uint256 productId, string name, uint256 weight);

    /// @notice Emitted when product items are added
    event ProductItemsAdded(string[] itemIds, uint256 productId);

    /// @notice Emitted when the status of product items is changed
    event ProductItemsStatusChanged(string[] itemIds, ProductStatus status);

    /// @notice Emitted when a new manufacturer registers
    ...
    /// @notice Registers a new manufacturer
    /// @param _name Name of the manufacturer
    /// @param _location Physical location of the manufacturer
    /// @param _contact Contact details for the manufacturer
    ...
    /// @notice Adds a new product and its toxic items
    /// @param _name Name of the product
    /// @param toxicNames Names of toxic items in the product
    /// @param toxicWeights Weights of the toxic items in the product
    function addProduct(
        string memory _name,
    ...
    /// @notice Adds product items to a specific product
    /// @param _productId ID of the product to add items to
    /// @param _quantity Number of product items to add (maximum 10)
    ...
    /// @notice Updates product items' status to Sold
    /// @param itemIds Array of product item IDs to update status
    ...
    /// @notice Updates product items' status to Returned
    /// @param itemIds Array of product item IDs to update status
    function returnProductItems(string[] memory itemIds) public {
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            require(
                msg.sender == products[productId].manufacturer,
                'Only the product manufacturer can return product items.'
            );
    ...
    /// @notice Updates product items' status to Recycled
    /// @param itemIds Array of product item IDs to update status
    function recycleProductItems(string[] memory itemIds) public {
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            require(
                msg.sender == products[productId].manufacturer,
                'Only the product manufacturer can recycle product items.'
            );
